...
export function userRoutes(app: Hono<{ Bindings: Env }>) {
    ...
    app.get('/api/auth/setup-status', async (c) => {
        const count = await db.countUsers(c.env.DB);
        return ok(c, { isSetup: count > 0 });
    });

    app.post('/api/auth/setup', async (c) => {
        const count = await db.countUsers(c.env.DB);
        if (count > 0) {
            return bad(c, 'Setup already completed. Please login.');
        }

        const { email, password, name, company } = await c.req.json();

        if (!isStr(email) || !isStr(password) || !isStr(name)) {
        ...
        // Insert user manually to include password_hash
    ...
    `).bind(
        ...
        const { password_hash: _, ...safeUser } = newUserAccount;
        return ok(c, safeUser as User);
    });
